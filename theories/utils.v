...
Section MapLemmas.
  ...
  (* Lemma map_add_comm: forall m key key' elt elt', *)
  ...
  (* Lemma fold_not_in: forall (G0:G) y (T_x_t_y_list: list (T*x*t*x)), *)
  (*     G0 !! y = None -> *)
  (*     ~ In y (map (fun '(_, _, _, y') => y') T_x_t_y_list) -> *)
  (*     (fold_right (fun '(T_, _, _, y) (G0 : G) => insert y (ctxv_T T_) G0) G0 T_x_t_y_list) !! y = None. *)
  (* Proof. *)
  (*   induction T_x_t_y_list as [ | (((?T_, ?x_), ?t_), ?y) T_x_t_y_list IH ]; [easy|]. *)
  (*   simpl; intros ? ?. *)
  (*   destruct (eq_x y y0); subst; simpl_map. *)
  (*   - exfalso. *)
  (*     apply H8; auto. *)
  (*   - apply Decidable.not_or in H8. *)
  (*     destruct H8. *)
  (*     specialize (IH H7 H9). *)
  (*     rewrite lookup_ne. *)
  (*     simplify_map_eq. *)
